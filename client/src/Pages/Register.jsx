...
const StudentRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const trimmedMatric = matric.trim().toUpperCase();
    const trimmedEmail = email.trim().toLowerCase();
    const trimmedPassword = password.trim();
    const trimmedDepartment = department.trim().toUpperCase();

    if (!trimmedFullname || !trimmedMatric || !trimmedEmail || !trimmedPassword || !trimmedDepartment) {
    ...
    if (trimmedMatric.length < 10) {
      toast.error('Matric number must be at least 10 characters');
      return;
    }
      ...
      <div className="form-container ">
        ...
        <form className="form" onSubmit={handleSubmit} noValidate>
          ...
          <input
            ...
            className="input"
            placeholder="e.g., NSU/NAS/CMP/0818/18/19"
            required
            disabled={loading}
