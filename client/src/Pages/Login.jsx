...
const StudentLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const trimmedMatric = matric.trim().toLowerCase();
    const trimmedPassword = password.trim();

    if (!trimmedMatric) {
      toast.error('Matric is required');
      return;
    }
    if (!trimmedPassword) {
    ...
    try {
      await studentLogin(trimmedMatric, trimmedPassword);
    } catch (error) {
    ...
    <StyledWrapper>
      ...
      <div className="form-container item">
        ...
        <p className="sign-up-label">
          ...
          <Link to="/student/register" className="sign-up-link">
            Register Now!
          </Link>
        </p>
        <p className="sign-up-label">
